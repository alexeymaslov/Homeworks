...
void breadthFirstSearch(int** table, int sizeOfGraph, bool* visited, int start)
{
    Queue *queue = createQueue();
    enqueue(start, queue);
    while (!isEmpty(queue))
...
int main()
{
    ...
    char address[sizeOfAddress] = "c:\\users\\alex\\8-3 test.txt";
    ...
    if (myFile)
    {
        ...
        for (int q = 0; q < size; q++)
            if (!isVisited[q])
            {
                cout << "Connected component No " << counter << ": ";
                breadthFirstSearch(graph, size, isVisited, q);
                cout << endl;
                counter++;
            }
